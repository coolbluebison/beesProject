...
export default function ProfileNav({pHover,setPHover,user,setUser}){
        ...
        <div className={pHover?"w-72 px-4 py-4 bg-white rounded-b-2xl absolute z-40 right-20 shadow-md":"hidden"} onMouseOver={()=>{setPHover(true)}} onMouseLeave={()=>{setPHover(false)}}>
            {/* put content here */}
                ...
                <div>
                    ...
                    <p className="p-2 px-4 text-gray-800 cursor-pointer w-full rounded-xl hover:bg-honey">Logout</p>
                </div> 
                :
                <div>
                    <p className="p-2 px-4 text-gray-800 cursor-pointer w-full rounded-xl hover:bg-honey" onClick={()=>nav("login")}>Log In</p>
                </div>   
            }
        </div>
    )
